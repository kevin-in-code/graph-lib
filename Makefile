...
$(MAKEFILES): | Makefile
	...
	@( $(foreach T,$(patsubst $(PROGDIR)/%/Makefile.inc,%,$@),echo '';) ) >>$@
	@( $(foreach T,$(patsubst $(PROGDIR)/%/Makefile.inc,%,$@),echo '$$(BUILDDIR)/$T/%.o: $$$$(GRAPHLIB) $$$$(PROGDIR)/$T/%.cpp $$(wildcard $$(PROGDIR)/$T/*.hpp) $$(wildcard $$(PROGDIR)/$T/*.h)';) ) >>$@
